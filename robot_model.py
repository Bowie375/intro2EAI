...
class RobotModel:
    ...
    def fk(self, qpos: np.ndarray) -> np.ndarray:
        ...
        # calculate the relative transformation bwtween all joint frames

        def get_transform(rotation:np.ndarray, translation:np.ndarray, parent_transform:np.ndarray) -> np.ndarray:
            transform = np.eye(4)
        ...
        q_idx = 0
        transformations = [np.eye(4)] # add base link's transform
        for i, joint in enumerate(self.joints):
            if i == 0:
                parent_transform = np.eye(4)
            else:
                parent_transform = transformations[-1]

            if isinstance(joint, FixedJoint):
                transformations.append(get_transform(joint.rot, joint.trans, parent_transform))
            elif isinstance(joint, RevoluteJoint):
                q = qpos[q_idx]
                q_idx += 1
                rotation = rpy_to_mat(np.array([0, 0, q]))
                joint_rot = joint.rot @ rotation
                transformations.append(get_transform(joint_rot, joint.trans, parent_transform))
