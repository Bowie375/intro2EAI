...
class RobotModel:
    def __init__(self, robot_cfg: RobotCfg):
        ...
        self.robot_head = rtb.ERobot.URDF(os.path.abspath(robot_cfg.head_path_urdf))
    ...
    def fk_link_head(
        ...
        fk = self.robot_head.fkine(qpos, end=link_name)
        return fk.A[:3, 3], fk.A[:3, :3]
    ...
    def fk_camera(self, qpos: np.ndarray, camera_id: int = 1) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        """
        if camera_id == 1:
            link_trans, link_rot = self.fk_link(qpos, self.cfg.camera_cfg[camera_id].link_name)
        elif camera_id == 0:
            link_trans, link_rot = self.fk_link_head(qpos, self.cfg.camera_cfg[camera_id].link_name)
        rel_cam_trans, rel_cam_rot = (
            self.cfg.camera_cfg[camera_id].extrinsics[:3, 3],
            self.cfg.camera_cfg[camera_id].extrinsics[:3, :3],
        )
    ...
    def ik_cam(
        ...
        This function only return the joint angles that affect the end-effector.
        """
        camera_pose = to_pose(trans, rot)

        rel_cam_trans, rel_cam_rot = (
            self.cfg.camera_cfg[1].extrinsics[:3, 3],
            self.cfg.camera_cfg[1].extrinsics[:3, :3],
        )
        link_pose = camera_pose @ np.linalg.inv(self.cfg.camera_cfg[1].extrinsics)

             
        for _ in range(retry_times):
            ik_result, success, _, _, _ = self.robot.ik_lm_chan(
                link_pose, end=self.cfg.camera_cfg[1].link_name, q0=init_qpos
            )
            if success:
                if delta_thresh is not None:
                    if np.linalg.norm(ik_result - init_qpos) > delta_thresh:
                        continue
                link_trans, link_rot = self.fk_link(ik_result, self.cfg.camera_cfg[1].link_name)
                cam_trans = link_trans + link_rot @ rel_cam_trans
                cam_rot = link_rot @ rel_cam_rot
                if np.linalg.norm(cam_trans - trans) < trans_tol and rot_dist(cam_rot, rot) < rot_tol:
                    return True, ik_result
        return False, ik_result
