...
class PoseDataset(Dataset):
    def __init__(self, config: Config, mode: str, scale: int = 1):
        ...
        for i in range(len(self.files)):
            path = os.path.join(self.data_root, self.files[i], "in.bin")
    ...
    def __getitem__(self, idx: Optional[int]) -> Dict[str, np.ndarray]:
        data = self._data[idx]
        if data is not None:
            return data
        else:
    ...
    def preload(self, idx: Optional[int]) -> Dict[str, np.ndarray]:
        """
        ...
        try:
            ...
            # trimesh.PointCloud(pc_world_obj).export("/root/workspace/Assignment4-final/tmp/pcd_std.ply")
            # cv2.imwrite(
