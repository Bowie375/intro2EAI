...
class WrapperEnv:
    def __init__(self, config: WrapperEnvConfig):
        ...
        self.sim = VircualSim()
        self.sim.humanoid_robot_cfg = self.humanoid_robot_cfg

        self.server = RobotServer()
        ws_thread = Thread(target=self.server.start, daemon=True)
        ws_thread.start()
    ...
    def launch(self):
        ...
        while True:
            is_connect = self.server.check_active_connections()
            if is_connect:
    ...
    def reset(self, humanoid_head_qpos: Optional[np.ndarray] = None, humanoid_qpos: Optional[np.ndarray] = None, reset_wait_steps: Optional[int] = None):
        ...
        self.server.send_humanoid_head(list(humanoid_head_qpos), True)
        self.server.send_humanoid_command([list(humanoid_init_qpos[:7])], True)
    ...
    def get_obs(self, camera_id: int = 1) -> Obs:
        ...
        data = self.server.get_latest_data()
        if camera_id == 0:
            rgb = data.head_camera.color_image
            # imageio.imwrite('out.png', rgb)
            depth = data.head_camera.depth_image
            print(data.head_camera.pose['position'], data.head_camera.pose['orientation'])
            cam_rot = R.from_quat(data.head_camera.pose["orientation"]).as_matrix()
            camera_pose = to_pose(data.head_camera.pose["position"], cam_rot)
        elif camera_id == 1:
            rgb = data.wrist_camera.color_image
            depth = data.wrist_camera.depth_image
            cam_rot = R.from_quat(data.wrist_camera.pose["orientation"]).as_matrix()
            camera_pose = to_pose(data.wrist_camera.pose["position"], cam_rot)
    ...
    def get_intrinsic(self, camera_id: int = 1):
        data = self.server.get_latest_data()
        if camera_id == 0:
            intrinsic = data.head_camera.intrinsics
        elif camera_id == 1:
            intrinsic = data.wrist_camera.intrinsics
    ...
    def step_env(
        ...
        if quad_command is None:
            ...
            self.server.send_quadruped_command(quad_command[0], quad_command[1], quad_command[2])
        ...
        if gripper_open is not None:
            self.server.send_humanoid_gripper(gripper_open, True)
        if humanoid_head_qpos is not None:
            self.server.send_humanoid_head(list(humanoid_head_qpos), True)
        if humanoid_action is not None:
            if type(humanoid_action) == np.ndarray:
                humanoid_action = humanoid_action.tolist()
            elif type(humanoid_action) == list:
                humanoid_action = [arr.tolist() for arr in humanoid_action]
            self.server.send_humanoid_command(humanoid_action, True)
