...
class GraspEnv:
    ...
    def export_obs(self, obs: Obs, idx:int, data_dir:str):
        """Save the observation to the specified directory."""
    ...
    def execute_plan(self, traj: np.ndarray) -> bool:
        ...
        output_dir=os.path.join("data", "outputs", time.strftime("%Y%m%d_%H%M%S"))
        for idx, qpos in enumerate(traj):
            self.sim.step(qpos[:8])
            self.export_obs(self.get_obs(), data_dir=output_dir, idx=idx)
