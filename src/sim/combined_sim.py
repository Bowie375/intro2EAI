...
class CombinedSim:
    ...
    def render(self, camera_id=0, camera_pose=None):
        ...
        for geom_name in self.geom_dict["power_drill"]:
            geom_id = self.mj_model.geom(geom_name).id
            mask[seg == geom_id] = 255
        ret["seg"] = np.expand_dims(mask, 2).repeat(3, axis=2).astype(np.uint8)
        return ret
