...
from src.driller_detector.config import Config
from src.driller_detector.logger import Logger
from src.utils import set_seed
from src.driller_detector.data_utils.data import Loader, PoseDataset
from src.driller_detector.model2 import get_model
...
def main(**args_override):
    ...
    # loading datasets
    train_dataset = PoseDataset(config, mode="train", scale=1)
    val_dataset = PoseDataset(config, mode="val", scale=1)
...
if __name__ == "__main__":
    ...
    #####################################
    #   Part 1: train est_pose model    #
    #####################################

    """ Uncomment Part 1 and comment Part 2 to train the est_pose model
    ...
    for i in range(1):
        ...
        args = dict(
            ...
            batch_size = bsz,
            use_mse_loss_on_rot = mse_rot,
            # checkpoint = "/root/workspace/Assignment4-final/exps/est_coord_bsz_16_lr_0.001_mse_rot_False_20250606_130446/checkpoint/checkpoint_60000.pth",
            # max_iter = 80000,
            device = f"cuda:{i}",
